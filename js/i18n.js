(function () {
  ...
  const translations = {
    en: {
      ...
      "about.desc1":
        "Darbak is a 100% Saudi luxury land transport company, dedicated to providing exceptional service to pilgrims, visitors, and all travelers to the Kingdom. We combine tradition with modernity to ensure every journey is safe, comfortable, and memorable.",
      "about.desc2":
        "We cover all routes: Jeddah ↔ Mecca ↔ Medina ↔ Taif ↔ Airports with a modern and comfortable fleet with professional drivers.",
      ...
      "why.desc":
        "Choose Darbak for an unparalleled transport experience that combines ultimate safety, luxury, and punctuality. With our modern 2025 fleet, professional drivers, and 24/7 customer service, every journey becomes a testament to excellence and meticulous care for every detail. We ensure your comfort and peace of mind throughout your trip.",
      ...
      "faq.a1":
        "Yes, we follow flights moment by moment and wait for you no matter how long the delay without any additional fees.",
      ...
      "faq.a3":
        "All our cars are modern, fully air-conditioned, clean, and comfortable for long and short trips.",
    ...
    ar: {
      ...
      "about.desc1":
        "دربك هي شركة نقل بري فاخر سعودية 100%، متخصصة في خدمة الحجاج والمعتمرين وزوار المملكة.",
      "about.desc2":
        "نغطي جميع المسارات الية: جدة ↔ مكة ↔ المدينة ↔ الطائف ↔ المطارات بأسطول حديث ومريح مع سائقين محترفين.",
      ...
      "why.desc":
        "نقدم لك تجربة تنقل راقية تجمع بين الأمان، الرفاهية، والالتزام بالمواعيد. كل رحلة معنا عنوان للتميز والعناية بأدق التفاصيل.",
      ...
      "faq.a1":
        "نعم، نتابع الرحلات الجوية لحظة بلحظة وننتظرك مهما طال التأخير بدون أي رسوم إضافية.",
  ...
  const i18n = {
    // ── State ────────────────────────────────────
    current: _resolveInitialLang(),

    // ── Core ─────────────────────────────────────
    t(key) {
      const lang = translations[this.current];
      return lang && lang[key] ? lang[key] : null;
    },
    ...
    apply() {
      ...
      if (meta) {
        const val = this.t("meta.description");
        if (val) meta.content = val;
      }

      // 6. Sync language toggle buttons
    ...
    init() {
      ...
      // Expose globally for inline usage (e.g. onclick handlers)
      window.i18n = this;
    },
  };
  ...
  function _bindLangButtons() {
    document.querySelectorAll(".lang-btn").forEach((btn) => {
      // Avoid duplicate listeners on re-init
      btn.removeEventListener("click", btn._i18nHandler);
      btn._i18nHandler = () => window.i18n.set(btn.getAttribute("data-lang"));
      btn.addEventListener("click", btn._i18nHandler);
    });
  }
